...
class getSHA(QThread):
    ...
    def gitLog(self):
        ...
        with open(Main.wrapper_path + "\log.txt", "w") as file:
            #file.write(logInfo)
            for line in logInfo:
                file.write(line)
                file.write("\n")
                    ...
                    if stopFlag == False:
                        ...
                        if scriptname != "":
                            ...
                            if "0-1_NVMe_Preparation" in file_path:
                                ...
                                # 執行開卡動作!
                                # 卡住不動5分鐘的話就timeout(不太穩定)
                                procress = subprocess.run([file_path, branch_input, SHA_input], stderr=subprocess.STDOUT, timeout=300) 
                                
                                                   
                                # 把decode()給拿掉就不會有AttributeError
                                str_List = str(procress.stdout).split('\r\n\r\n')
                                #str_List = str(procress.stdout.decode('cp950')).split('\r\n\r\n')  # i.replace("\r\n", "")  #原始版本，替換特定字串用(here are problem!!!)
                                
                                #print(str_List)
                                
                                # Write batch output into CMD_MSG.log (存取開卡寫入)
                                ...
                                if Main.checkBox.isChecked():   #if global checkbox is true...
